function prompt_login --description "display user name for the prompt"
    ...
    if set -q SSH_TTY; and set -q fish_color_host_remote
        ...
        echo -n -s (set_color $fish_color_user) "$USER" (set_color normal) @ (set_color $color_host) (prompt_hostname) (set_color normal) " in" 
    else
        echo -n -s (set_color $fish_color_user) "$USER" (set_color normal) " in" 
    end
